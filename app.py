'''
File: app.py
Author: @sha0coder, @Landen-Martin
Description: Python3 to nasm 64bits

Disclaimer!!! Never use this to create malicious payloads!
'''
...
def find_locals(node):
    """grab all assigned names that arent regs"""
    found = []
    for n in ast.walk(node):
        if isinstance(n, ast.Assign):
            for t in n.targets:
                if isinstance(t, ast.Name) and t.id not in regs64 and t.id not in found:
                    found.append(t.id)
    return found
...
class visit_functions(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        global nasm, locals_map, local_slot
        nasm.append(f'\n{node.name}:')

        # x64 dont have pushad, do it manualy
        for r in save_regs:
            nasm.append(f'  push {r}')
        ...
        for lv in find_locals(node):
            local_slot += 8
            locals_map[lv] = local_slot

        if local_slot > 0:
            nasm.append(f'  sub rsp, {local_slot}')
    ...
    def visit_Return(self, node):
        ...
        # popad manual, rax stays for return value
    ...
    def visit_Assign(self, node):
        ...
        for target in node.targets:
            if target.id in locals_map:
                slot = locals_map[target.id]
                if isinstance(node.value, ast.Constant):
                    nasm.append(f'  mov rax, {node.value.value}')
                    nasm.append(f'  mov [rbp-{slot}], rax')
                elif isinstance(node.value, ast.Name):
                    if node.value.id in locals_map:
                        nasm.append(f'  mov rax, [rbp-{locals_map[node.value.id]}]')
                        nasm.append(f'  mov [rbp-{slot}], rax')
                    else:
                        nasm.append(f'  mov [rbp-{slot}], {node.value.id}')
            ...
            if isinstance(node.value, ast.Constant):
                ...
                # put bytes on stack, reg points to first byte
                data = b''
                for e in node.value.elts:
                    if isinstance(e, ast.Constant):
                        data += bytes([e.value & 0xff])
                    else:
                        unimplemented('array elem')

                while len(data) % 8:
                    data += b'\x00'

                # push qwords in reverse so first bytes end up at rsp
                for i in range(len(data) - 8, -8, -8):
                    chunk = data[i:i+8]
                    val = int.from_bytes(chunk, 'little')
                    nasm.append(f'  mov rax, 0x{val:016x}')
                    nasm.append(f'  push rax')

                nasm.append(f'  mov {target.id}, rsp')
